...
[//]: # (    - For Mac and Linux: `ssh root@pekascookies.com`)
    -For Mac and Linux: 'ssh root@pekascookies.com
...
[//]: # (    - `useradd -m -s /bin/bash -g users -G sudo ptrujillo`)
    - - `useradd -m -s /bin/bash -g users -G sudo ptrujillo`
    - replace ptrujillo with your own
- create password
    - `passwd ptrujillo` passwd ptrujillo`
- Move .ssh key file
    - `cp -r .ssh /home/ptrujillo`
- change ownership of the .ssh directory
    - `cd /home/ptrujillo`
    - `chown -R ptrujillo:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh ptrujillo@pekascookies.com`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- reboot
    - `sudo reboot`
- ssh back in
- `ssh ptrujillo@pekascookies.com`
...
- create a directory for your deployment
    - `sudo mkdir -p /var/www/pekascookies.com/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/pekascookies.com/html`
...
- `sudo vim /etc/nginx/sites-available/pekascookies.com`
...
server {
    ...
    root /var/www/pekascookies.com/html;
    index index.html index.htm index.nginx-debian.html;

    server_name pekascookies.com www.pekascookies.com;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/pekascookies.com /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/ptrujillo/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/ptrujillo/.ssh/github_deploy_key.pub >> /home/ptrujillo/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/ptrujillo/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H pekascookies.com >> ~/.ssh/known_hosts 
         scp -r ./dist/* ptrujillo@pekascookies.com:/var/www/pekascookies.com/html
```
